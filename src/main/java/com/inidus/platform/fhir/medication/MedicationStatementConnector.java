...
//    }
    ...
    private String getdateStartedFilterAql(DateRangeParam dateStarted) {
        ...
        if (null != fromDate) {
            ...
            filter += String.format(" and b_a/activities[at0001]/description[at0002]/items[at0113]/items[at0012]/value/value as Order_start_date = '%s'", from);
        }

        Date toDate = dateStarted.getUpperBoundAsInstant();
        if (null != toDate) {
            String to = ISO_DATE.format(toDate);
            filter += String.format(" b_a/activities[at0001]/description[at0002]/items[at0113]/items[at0012]/value/value as Order_start_date <= '%s'", to);
        }
        return filter;
    }
